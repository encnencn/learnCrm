...
$.fn.extend({  
    ...
    initPage : function(totalPage,currentPage,pageSize,targetForm) {
    	//alert("totalPage:"+totalPage+",currentPage:"+currentPage+",pageSize:"+pageSize);
         ...
         */  
        if (currentPage <= 1) {  
            pageHtml += '<a class="no" href="javascript:;" style="margin: 5px;" >上一页</a>';  
        } else {  
            pageHtml += '<a href="javascript:$.fn.goPage('+(currentPage-1)+');" style="margin: 5px;" >上一页</a>';  
        }  
        if (totalPage <= limitSize) {  
            for (var i = 1; i <= totalPage; i++) {  
                ...
                /** *选中当前页** */  
                if (currentPage == i) {  
                    pageHtml += ' style="text-decoration:underline ;margin: 5px;" ';  
                }  
        ...
        if (currentPage == totalPage) {  
            pageHtml += '<a class="no" href="javascript:;" style="margin: 5px;">下一页</a>';  
        } else {  
            pageHtml += '<a href="javascript:$.fn.goPage('+(currentPage+1)+');" style="margin: 5px;">下一页</a>';  
        }  
        pageHtml += ' 转到：<cite class="wsc-form-cite a text"><input type="text" id="page" name="page" style="width: 40px;height:25px" value="" inited="true" ></cite> 页';  
        pageHtml += '<a href="javascript:;" class="_jump_page_link" style="margin: 5px;">确定</a>';  
    ...
    goPage:function(nextPage){  
        ...
        if($.fn.targetForm.length){  
            ...
            var params = _pageUrl.split("html");
            
            //alert("要跳转的页面："+nextPage);
          
           window.location.href ="operatorLog.html?pageNum="+nextPage;
            //window.location.href = params[0]+"logList2?pageNum="+nextPage;
           // $.logManager.loadLog(nextPage,'');
           // $(obj).buildPage(result.pageNum,result.pageSize,result.pages); 
           // alert(params[0]+"logList2?pageNum="+nextPage);
        }  
    },  
    /** 
     * ajax分页回调 
    ...
    buildPage:function(pageNum,pageSize,total){  
        ...
        if (currentPage <= 1) {  
            ...
            pageHtml += '<a href="javascript:$.fn.goAjaxPage('+(currentPage-1)+');" style="margin: 5px;">上一页</a>';  
        }  
        if (totalPage <= limitSize) {  
            for (var i = 1; i <= totalPage; i++) {  
                pageHtml += '<a ';  
                /** *选中当前页** */  
                if (currentPage == i) {  
                    pageHtml += ' class="active" style="margin: 5px;"';  
                }  
        ...
        if (currentPage == totalPage) {  
            ...
            pageHtml += '<a href="javascript:$.fn.goAjaxPage('+(currentPage+1)+');" style="margin: 5px;">下一页</a>';  
        }  
