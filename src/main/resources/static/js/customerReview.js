...
$(function() {
	var customerManager = {
		...
		init : function() {
			...
			this.applicationRefused();//申请拒绝
			this.applicationApproved();//申请通过
			this.detailConfirm()//查看详情后点击确定
		},
							...
							var thisDate = function(curr) {
								...
								for (var i = (curr * nums - nums); i <= last; i++) {
									...
									str += '<tr class="role-tr">'
											...
											+ '<button class="btn btn-xs btn-primary btn-detail-view" data-toggle="modal" data-target="#myModal4">'
											...
											+ '月报'
											+ '</button>'*/
											+ '<button class="btn btn-xs btn-success btn-approved" '+display+' data-toggle="modal" data-target="#myModal5">'
											+ '<i class="ace-icon fa fa-pencil bigger-120" ></i>'
											+ '审核通过'
											+ '</button>'
											+ '<button class="btn btn-xs btn-danger btn-refused" '+display+'  data-toggle="modal" data-target="#myModal5">'
											+ '<i class="ace-icon fa fa-pencil bigger-120" ></i>'
											+ '审核拒绝'
											+ '</button>'
											+ '</div>'
		...
		detailConfirm : function() {
			var that = this;
			$(".modal-footer-customer-detail").on(
					'click',
					'.btn-primary',
					function() {
		...
		applicationApproved : function() {
			var that = this;
			$("#role-manage-role").on('click', '.btn-approved', function() {
				// 要删除的索引 删除DOM树
		...
		//审核拒绝
		applicationRefused : function() {
			var that = this;
			$("#role-manage-role").on('click', '.btn-refused', function() {
				// 要删除的索引 删除DOM树
		...
		searchCustomer : function() {
			...
			$(".modal-footer-search").on('click','.btn-customer-search',function() {
						...
						var customername = $(this).parents('.modal-content-search')
								.find('#form-field-1').val();
						if(customername==""){
