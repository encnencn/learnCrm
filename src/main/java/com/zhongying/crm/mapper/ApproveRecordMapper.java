...
@Select("select approverecord.*,admin.trueName,customer.customerName from approverecord left join admin on "
		+ "approverecord.adminId=admin.id left join customer on approverecord.customerId= customer.id where "
		+ "approverecord.approveDate BETWEEN #{startDate} AND #{endDate} and approverecord.customerName LIKE CONCAT(CONCAT('%',#{customerName},'%'))")
public List<ApproveRecord> queryRiZhi1(@Param("startDate")String startDate,@Param("endDate") String endDate, @Param("customerName")String customerName);
